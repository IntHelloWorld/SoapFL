...
from myparser.language_processors.utils import get_tokens
...
class RubyProcessor:
    ...
    def extract_if_else(cls, root, code_str, operator_list):
        ...
        while len(queue) > 0:
            current_node = queue[0]
            queue = queue[1:]
            if (str(current_node.type) == 'if' or str(current_node.type) == 'unless') and len(
                    code_str[current_node.start_byte:current_node.end_byte].decode()) > 6:
